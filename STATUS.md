...
「根拠の可視化」タブを強化。積み上げ棒グラフの下に、**各馬の3要素ごとの評価理由を
日本語の短文**で表示するようにしました（`st.expander`、上位3頭は⭐+デフォルト展開）。
...
# → サイドバーで 5/31 東京 11R(日本ダービー) を選択 → 「根拠の可視化」タブ
```

実データでの出力例（ダービー1位）:
- 📊 トラックバイアス `-3.4`：内枠（3枠）×先行 → 外有利・差し有利の馬場に対し不利寄り
- 🧬 血統 `+13.6`：父コントレイル 芝2400m前後 勝率21%（n=28）。サンプル少なめ→ベイズ補正
- 🏃 脚質 `+8.8`：先行（直近3走から推定）。想定スローペース → 先行有利

スコアは中立(50)からの差で色分け（+10↑緑 / -10↓赤 / それ以外グレー）。データ不足は
「データ不足のため中立評価」等を明示。テスト: `test_reasoning.py` 18件 + smoke 4件。
